...
# 
# COPYRIGHT
#     Copyright 2012-2015 Ryan Reece
...
PRINT_WARNINGS = $(PRINT) "warnings/errors   -------------------------------------------------"; \
	$(CHECK_WARNINGS); \
	$(PRINT) "-------------------------------------------------------------------"
CHECK_TODOS = egrep -i -B 1 -A 1 '(^!)|(TODO)' $(SRC) $(INCLUDED_TEX) > todos.log
PRINT_TODOS = $(PRINT) "checking TODOs: logged in todos.log"; \
	$(CHECK_TODOS)
...
draft: pdf
	@echo "making draft: drafts/$(DRAFT)" ; \
	mkdir -p drafts/ ; \
	echo "cp $(PDF) drafts/$(DRAFT)" ; \
	cp $(PDF) drafts/$(DRAFT)
...
bib: $(SRC) $(INCLUDED_TEX)
	$(RUN_BIBTEX)

one: $(SRC) $(INCLUDED_TEX)
	$(RUN_LATEX_FIRST)
